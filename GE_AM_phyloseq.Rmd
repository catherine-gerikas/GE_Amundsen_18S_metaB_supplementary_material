---
title: "Phytoplankton spring bloom diversity across the marginal ice zone in Baffin Bay, Arctic"
...
> Procotol used to generate data for the paper "Phytoplankton spring bloom diversity across the marginal ice zone in Baffin Bay, Arctic". 
...
```{r importing_table}
  ...
  SAMPLE_tb <- read.table("SAMPLE_DATA_table.csv", header = TRUE, sep = ";", quote = "\"", dec = ",", fill = TRUE, comment.char = "", row.names=1)
...
```{r extract_photo}
  ...
  physeq_photo2 <- subset_taxa(physeq_nonDino, (division %in% c("Chlorophyta", "Cryptophyta", "Haptophyta", "Ochrophyta", "Cercozoa")) & !(class %in% c("Syndiniales", "Sarcomonadea", "Chrysophyceae", "Filosa-Thecofilosea", "Filosa-Imbricatea", "Filosa-Granofilosea", "Chlorarachniophyceae")))
...
```{r size_fractions}
  ...
  physeq_0_2um_b <- subset_samples(physeq_photo2, size_fraction %in% ("0-2um"))
  physeq_3um_b <- subset_samples(physeq_photo2, size_fraction %in% ("3um"))
  physeq_20um_b <- subset_samples(physeq_photo2, size_fraction %in% ("20um"))
...
Select taxa comprising the 90% most abundant sequences in at least 1 sample on each size fraction. For selection disregarding size fraction it was used 1/10 of the samples to include less taxa.
...
```{r abundant_taxa}
  ...
  nsamples_whall <- (1/10*nsamples(physeq_photo2))
  whall <- genefilter_sample(physeq_photo2, f1_b, A=nsamples_whall) 
  physeq_photo2_abund <- prune_taxa(whall, physeq_photo2)

# Another function for upset images

  f1_c <- filterfun_sample(topf(0.7)) #creates a function to keep 70% most abundant sequences
  whall_up <- genefilter_sample(physeq_photo2, f1_c, A=1) 
  physeq_photo2_abund_upset <- prune_taxa(whall_up, physeq_photo2)
...
```{r tax_glom}
  ...
  TG_division <- tax_glom(physeq_photo2_abund, "division")
  TG_class <- tax_glom(physeq_photo2_abund, "class")
  TG_genus <- tax_glom(physeq_photo2_abund, "genus")
...
```{r melt_phyloseq}
  ...
  melt_physeq_photo2 <- psmelt(physeq_photo2) 
  melt_physeq_photo2_abund <- psmelt(physeq_photo2_abund)
  melt_physeq_photo2_abund_upset <- psmelt(physeq_photo2_abund_upset)
...
melt_TG_division$fraction_name <- factor(melt_TG_division$fraction_name, levels = c("0.2-3 µm", "3-20 µm", "> 20 µm"))
melt_TG_class$fraction_name <- factor(melt_TG_class$fraction_name, levels = c("0.2-3 µm", "3-20 µm", "> 20 µm"))
melt_TG_genus$fraction_name <- factor(melt_TG_genus$fraction_name, levels = c("0.2-3 µm", "3-20 µm", "> 20 µm"))
...
ggsave("./IMAGES/figure_barplot_division.png")
...
ggsave("./IMAGES/figure_barplot_genus.png")
...
genus_abund_table <- subset(melt_physeq_photo2_abund, 
                     select = c("OTU", 
                                "Sample", 
                                "Abundance", 
                                "genus"))
...
ggsave(filename = paste("figure_FCM_", i, ".png", sep=""), path ="./IMAGES", dpi = 300, height = 5, width = 7)
...
upset_MERGED_ALL_upset <- upset_df_maker_function(physeq_photo2_abund_upset)
upset_MERGED_ALL <- upset_df_maker_function(physeq_photo2)
...
ggsave("./IMAGES/figure_count_asvs.png", dpi = 300, width = 3, height = 4)
...
site.scrs$fraction_name <- factor(site.scrs$fraction_name, levels = c("0.2-3 µm", "3-20 µm", "> 20 µm"))
...
NMDS_vectors_plot_genus_sections_V2 +
 ...
 NMDS_vectors_plot_genus_sections_V2 +
  ...
  ggsave("./IMAGES/NMDS_vectors_env.png")
...
# order factors
  ...
  physeq_readsmin_richness@sam_data$fraction_name <- factor(physeq_readsmin_richness@sam_data$fraction_name, levels = c("0.2-3 µm", "3-20 µm", "> 20 µm"))
...
rich01 + geom_point(size=3, alpha=0.5, width = 0.2, position = position_jitter(w = 0.1, h = 0.1)) +
  ...
  ggsave("./IMAGES/figure_alpha_richness.png", dpi = 300)
...
ggsave(filename = paste("figure_env_", i,".png", sep=""), path = "./IMAGES", dpi = 300)
